...
const filter__bar = ["catalog", "type", "country", "volume", "product"];

filter__bar.map((item) => {
	for (let i = 1; i <= $(`.bar-block__filter-${item}-c`).length; i++) {
		$(`#bar_${item}_${i}`).change(function () {
			if (this.checked) {
				$(`.bar-block__filter-${item}-c:nth-child(${i})`).addClass(
					`bar-block__filter-${item}-c__a`
				);
			} else {
				$(`.bar-block__filter-${item}-c:nth-child(${i})`).removeClass(
					`bar-block__filter-${item}-c__a`
				);
			}
		});
...
$(".bar-block__filter-search").on("click", () => {
